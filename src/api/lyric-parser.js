...
export default class Lyric {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (txt) {
        if (result[3].length === 3) {
          result[3] = result[3] / 10; // [00:01.997]中匹配到的997会被切成99
        }
        this.lines.push({
          time:
  ...
  play(offset = 0, isSeek = false) {
    ...
    // 现在处于第this.curLineIndex - 1行
    // 立即定位, 方法是调用传来的回调函数, 并把当前歌词信息传给他
    this._callHandler(this.curLineIndex - 1);
    // 根据时间进度判断歌曲开始的时间戳
    this.stateStamp = +new Date() - offset;
